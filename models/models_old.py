...
class Group(Base):
    ...
    group_id = Column(Integer, ForeignKey('user.user_id', ondelete="CASCADE"), primary_key=True)
    ...
    def __repr__(self):
        return "<Group(group_id='%s', members_count='%s')>" % (
                                self.group_id, self.members_count)
...
class Student(Base):
    ...
    def __repr__(self):
        return "<Student(student_id='%s', facultyId='%s', course='%s', groupId='%s')>" % (
...
class Teacher(Base):
    ...
    teacher_id = Column(Integer, ForeignKey('user.user_id', ondelete="CASCADE"), primary_key=True)
    ...
    def __repr__(self):
        return "<Teacher(teacher_id='%s', departmentId='%s', teacherId='%s')>" % (
                                self.teacher_id, self.departmentId, self.teacherId) 
...
class Action(Base):
    ...
    def __repr__(self):
        return "<Action(id='%s', user_id='%s', action_type='%s', comment='%s', time='%s')>" % (
...
class Schedule(Base):
    ...
    def __repr__(self):
        return "<Schedule(schedule_id='%s', date='%s', schedule_text='%s')>" % (
                                self.schedule_id, self.date, self.schedule_text)


class Timer(Base):
    __tablename__ = 'timer'

    id = Column(Integer, ForeignKey('user.user_id', ondelete="CASCADE"), primary_key=True)
    hour = Column(Integer)
